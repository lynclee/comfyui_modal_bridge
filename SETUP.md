...
四档 worker(CPU / L40S / H100 / B200)**一次部署全部建好**,空闲各自 scale-to-zero —— 没被路由到的档 **0 容器 = 0 成本**,所以多档不额外花钱。**切档不需要重新部署** —— 四档 worker 一次建好,选哪档只是运行时路由,选完下次提交即生效;只有改某档具体绑哪张卡(cheap_gpu / default_gpu / top_gpu)才要重新推送一次。
...
└── modal_app/
    ...
    ├── snapshot_bench.py     内存快照隔离 bench(开发专用，仅源码仓库提供)
...
└── modal_app/
    ...
    ├── snapshot_bench.py     isolated memory-snapshot bench (development only; source checkout)
