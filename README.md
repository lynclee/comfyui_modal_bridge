...
- **GPU 四种模式 + 自动按显存选卡(省钱)**:Modal Setup 里选 **Auto(更省钱,默认)**、**H100(固定 · 80G)**、**H200(固定 · 141G)** 或 **B200(固定 · 180G · 最快最强)**。Auto 按工作流估算显存自动选最省又够用的卡 —— 小图(如 **Z-Image-Turbo**)走 **L40S**、常规(如 **FLUX.2-dev**)走 **H100/A100**、真超主卡显存才升顶配档(防 OOM);固定档一律用所选那张。**显存不够会退化成频繁 offload(慢几倍),但显存够了之后再加大对速度帮助有限** —— 别只盯着最贵那档。点 RunModal 前还会按类别估算显存预警(视频含多帧激活)。四档 worker(CPU/省钱档/主卡/顶配档)**一次部署全部建好**,空闲各自 scale-to-zero,**没被路由到的档 0 成本**。
...
- **Four GPU modes + auto VRAM-based card pick (cost-saving)**: in Modal Setup choose **Auto (cheaper, default)**, **H100 (fixed · 80G)**, **H200 (fixed · 141G)** or **B200 (fixed · 180G · fastest)**. Auto estimates each workflow's VRAM and picks the cheapest card that fits — small images (e.g. **Z-Image-Turbo**) → **L40S**, normal (e.g. **FLUX.2-dev**) → **H100/A100**, above the primary card's VRAM it escalates to the top tier (avoids OOM); a fixed mode always uses the card you picked. **Too little VRAM degrades into constant offloading (several times slower), but past the point where the model fits, more VRAM buys little speed** — don't just pick the priciest tier. Before running, VRAM is estimated per category (video includes multi-frame activations) and warned on. All four workers (CPU / cheap / primary / top) are **deployed at once**, each scales to zero when idle — **an un-routed tier costs $0**.
