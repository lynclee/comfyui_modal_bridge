...
- **SageAttention lossy speedup (optional, off by default)**: turn it on in **Settings → Modal Bridge → Advanced**, then hit Deploy in Setup (the flag is baked into the image env; only the last layer rebuilds, seconds). QK matmuls run in INT8 — attention is ~70% of per-step FLOPs on long video sequences. Measured on MiniMax H3 @H100: sampling 48.6→24.6 s/it, **~−45% end-to-end**, no visible quality difference in same-seed A/B; image models (FLUX.2 etc.) have short sequences, gains are single-digit and composition may drift (turn it off to reproduce older outputs). Works on both the standard H100 and cheap L40S tiers (measured on L40S: 88→50.6 s/it, ~−43%; per-clip cost roughly matches H100+sage at ~2.2× the wall clock) — the prebuilt **dual-arch** wheel (sm_89+sm_90) is self-hosted in this repo's Release `sage-2.2.0-d1a57a5-multiarch`, avoiding the silent numerical bug in upstream's v2.2.0 tag; the top B200 tier (sm_100) cleanly falls back to SDPA.
